...
The intended direction is executable packages to internal orchestration packages, orchestration packages to domain/runtime packages, and concrete infrastructure hidden behind small interfaces. `internal/server` may depend on `internal/db`, `internal/objectstore`, `internal/models`, and `internal/web`; `internal/objectstore` must not depend on `internal/server`; `internal/db` must not depend on HTTP, CLI, or template packages.
...
Prefer package-boundary changes that reduce coupling over broad rewrites. Keep handlers thin when possible, push storage rules into `internal/objectstore`, persistence rules into `internal/db`, and runtime executable concerns into `internal/peekd`. Add tests near the package that owns the invariant.
